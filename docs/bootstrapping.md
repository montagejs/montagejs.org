...
layout: docs
title: Mod Bootstrapping

---

# Mod Bootstrapping

This document will describe the bootstrapping process for Mod.
...
First we set up the config object to load the Mod package. This involves setting up the loader that lets the us load ".mod" files directly (e.g. `require("mod/ui/text.mod")`), and compilers that attach Mod metadata to the exports of any loaded module (`SerializationCompiler`) and export the HTML of loaded HTML files as `content` (`TemplateCompiler`).

<a id="un-mopped-load-montage" href="#mopped-callback">If mopped, bundles are loaded at this point.</a>

We then use `Require` to load the Mod package. Once this promise has completed we have the require function for Mod, `montageRequire`. We use this to load the Q (Promise) package so that we have complete information about the Promise package. We then insert the already loaded promise module into it, so that it isn't requested again. We set up the linter, to give us informative errors when there's a syntax error in a loaded file.

At this point there is code to handle controlling the Mod bootstrapper from a remote frame, for example for testing. This won't be covered here.
...
Once this is complete we have the `montageRequire` and the `applicationRequire` and we use these to finish initializing Mod in `initMontage`

### initMontage

Here we load the last of Mod's essential dependencies and once they have completed we configure the application. This involves setting up the stack trace length (set to 0 for optimization), event manager and calling `montageWillLoad`, again for testing.

Next we check to see if the `package.json` specified an application prototype to use, otherwise we use "core/application". Once this is loaded will call `_load` on the application object which in the default implementation loads the Mod component and template modules, and causes any serialization in this HTML page to be parsed.
...
Before loading the Mod package the `BUNDLE` variable is checked. If it exists then a script tag is injected for each of the filenames in the array. Each bundle calls a global `bundleLoaded` function with its name. A `preloaded` promise is added to the config object that is resolved once all of the bundles have loaded. Mr waits on this promise before proceeding, which means that the Mod package is not loaded until all the bundles have loaded.
